...
def parse_config():
    ...
    parser.add_argument('--grad_accum_nums', type=int, default=4, help='Number of gradient accumulation steps. Effective batch size = batch_size * grad_accum_nums. Default=4 for better training stability.')
...
def main():
    ...
    # 梯度累积信息
    if args.grad_accum_nums > 1:
        effective_batch_size = args.batch_size * args.grad_accum_nums * (total_gpus if dist_train else 1)
        logger.info('******* Gradient Accumulation Enabled *******')
        logger.info('  grad_accum_nums: %d' % args.grad_accum_nums)
        logger.info('  effective_batch_size: %d (batch_size=%d x accum=%d x gpus=%d)' % (
            effective_batch_size, args.batch_size, args.grad_accum_nums, total_gpus if dist_train else 1))
    ...
    train_model(
        ...
        grad_accum_nums=args.grad_accum_nums
    )
